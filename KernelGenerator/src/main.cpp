...
int setupLoggerH(po::variables_map &vm);
int setupConfigdataH(po::variables_map &vm);
int setupShmHandlerH(po::variables_map &vm);
int setupRasppidataH(po::variables_map &vm);
...
//Sebastian Pack
int setupHeaderfiles(po::variables_map &vm){
    setupLoggerH(vm);
    setupConfigdataH(vm);
    setupRasppidataH(vm);
    setupShmHandlerH(vm);
...
{
    std::stringstream text;
    text << "#ifndef RASPPIDATA_H" << std::endl <<
            "#define RASPPIDATA_H" << std::endl <<
            "class RaspPiData" << std::endl <<
            "{" << std::endl <<
            "public:" << std::endl <<
            "    RaspPiData();" << std::endl <<
            "};" << std::endl <<
...
int setupShmHandlerH(po::variables_map &vm)
{
    ...
    text <<"#ifndef SIMPLESHAREDMEMORYHANDLER_H" <<std::endl <<
           ...
           "#include <string>" << std::endl <<
           "#include <iostream>" << std::endl <<
           "#include \"rasppidata.h\"" << std::endl <<
           "class SimpleSharedMemoryHandler" << std::endl <<
           "{" << std::endl <<
           "public:" << std::endl <<
           "    SimpleSharedMemoryHandler(std::string sharedMemoryName, int size);" << std::endl <<
           "    ~SimpleSharedMemoryHandler();" << std::endl <<
           "    void setData(std::string varName, std::__cxx11::string data);" << std::endl <<
           "private:" << std::endl <<
           "    boost::interprocess::managed_shared_memory* mySharedMemory;" << std::endl <<
           "    std::string sharedMemoryName, raspPiname;" << std::endl <<
           "};" << std::endl <<
